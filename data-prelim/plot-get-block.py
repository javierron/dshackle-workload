...
for target in ['besu', 'geth', 'dshackle']:
    for exp in ['sequential', 'random', 'cache']:
        # load the dataset
        dfx = pd.read_csv(f"block-read-{target}-{exp}.csv")
        dfx.columns = ["method", "block_number", "latency"]
        dfx.insert(0, "client", target, True)
        dfx.insert(0, "exp", exp, True)

        dfs.append(dfx)

df = pd.concat(dfs)
df['block_number'] = df['block_number'].map(lambda x: int(x, 0))
